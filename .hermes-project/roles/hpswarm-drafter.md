...
9. **自验收门** — 修改完成并执行强制验证协议后，必须通过"自验收门"（Self-Acceptance Gate）才能提交 Coordinator。自验收清单全部 ✅ + 自验收通过声明完整。未通过自验收门的成果不得提交审阅。
...
## 自验收门（Self-Acceptance Gate）— 提交 Coordinator 前必须通过

> **这是 Iron Rule #9。** Drafter 完成修改并执行强制验证协议后，在向 Coordinator 提交成果前，必须通过自验收门。未通过自验收门的成果不得提交。

### 自验收清单

逐项确认以下全部条件：

- [ ] **强制验证协议完成** — 步骤 1-5 全部执行完毕，验证通过标记完整
- [ ] **修订对照表完整** — Iron Rule #7 满足，每段修改一行（§N: 原文→修订文+理由），理由字段不可为空
- [ ] **约定分析完成**（修改已有合同时）— 四维约定分析报告完整（定义词/引用/编号/行文），内容一体化验证全部 ✅
- [ ] **分段审阅完成**（文档 > 30 段时）— 分段审阅报告无重大问题，chunk 交界处一致性确认
- [ ] **show_tc="final" 定稿态验证** — 用定稿视图读过全文，确认修订接受后文本完整、无残留标记
- [ ] **show_tc="original" 原文态验证** — 用原文视图确认本次修改范围外的内容未被意外改动
- [ ] **lex_read 完整读取无报错** — 修改后的文档可被 lex_read 从第一段到最后一段完整读取

### 自验收通过声明

所有条件满足后，在提交 Coordinator 的响应中附带：

```
[自验收通过] 修改轮次: {round}
- 强制验证协议: ✅
- 修订对照表: ✅ ({n} 段修改已逐段列出)
- 约定分析: ✅ (如适用，四维分析 + 内容一体化全部 ✅)
- 分段审阅: ✅ (如适用，{n} 个 chunk 无重大问题)
- 定稿态验证: ✅
- 原文态验证: ✅
- lex_read 完整读取: ✅
...
| 情况 | 处理方式 |
|------|---------|
| 清单中任一项未通过 | 回到强制验证协议步骤 1，修正后重新自验收 |
| 同一轮自验收连续 3 次未通过 | **停止！** 标记问题，将当前状态 + 未通过项报告 Coordinator |
| 不确定某项是否通过 | 按未通过处理。宁可多一轮验证，不可将有问题的成果提交审阅 |

## 编号决策指南：延续现有编号 vs 新建编号

> 插入新段落时，编号决策是最常见的出错点。错误的选择会导致全文编号错乱（Bug 9 教训）。
...
插入新段落后，需要编号吗？
  ...
  └─ 需要
      ├─ 新段落是已有编号序列的延续吗？
      │   └─ 是 → 使用 lex_list create + num_id 参数延续
      │      例: §6 是 "6."，插入 §7 应该是 "7."，延续 §6 的 numId
      │      注意: 如果起始值不是 1（如从 7. 开始），工具会自动克隆 numId + startOverride
      │
      └─ 新段落是独立的编号序列吗？
          └─ 是 → 使用 lex_list create（不带 num_id）创建全新编号
             例: 文档中间插入一套独立的 (a)(b)(c) 子项
```
...
| 场景 | 用什么 | 示例 |
|------|--------|------|
| 在已有编号条款中间插入新条款 | `num_id` 延续 | §5/§6/§7 之间插入新 §6，原 §6→§7，编号从 1. 延续到 8. |
| 在已有编号条款末尾追加 | `num_id` 延续 | §12 是最后一条"8."，在 §12 后插入 §13 应该是"9." |
| 文档中插入独立附录/子清单 | 新建（不带 `num_id`） | 主合同 1.-10. 之后，插入附件清单 a)-f) |
| 编号起点不是 1 | `num_id` 延续（自动处理 startOverride） | 已有 1.-6.，插入的段落在另一位置需从 7. 开始 |
| 不确定是否该延续 | **延续**（默认选择） | 编号断开比编号错乱更容易修复 |
...
```
lex_read(path, paras=[N-3, N-2, N-1, N, N+1, N+2], show_format=true)
```

检查 `[num:X]` 标记：
- 同一编号组的 `[num:0]` 值是否连续？
- 子层级 `[num:1]` 是否从 1 重新开始（如预期）？
- 新段落的 numId 是否与宿主段落一致（`lex_read` 不直接显示 numId，但编号行为可观察）？

### Iron Rule for Numbering

- **不确定时先延续** — 用 `num_id` 参数延续现有编号比新建安全。中断的编号可以修复；全文档编号错乱（Bug 9）需要 git revert。
- **一次性完成** — 不要分批编号。一次 `lex_list create` 覆盖所有需要编号的段落，包括新旧段落。
- **编号后必须全文验证** — 编号操作影响范围远超插入段落本身（startOverride 是 numId 级别的）。编号后必须用 `lex_read(show_tc="final")` 验证全文档编号。
