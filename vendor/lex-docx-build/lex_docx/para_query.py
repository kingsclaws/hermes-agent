...
def _build_style_map(doc) -> dict[str, dict]:
    """从 styles.xml 构建样式映射；兼容 python-docx 与 OpenXML wrapper。"""
    raw: dict[str, dict] = {}
    based_on: dict[str, str] = {}
    id_to_name: dict[str, str] = {}

    def _extract(el, name: str):
        attrs: dict = {}
    ...
    if hasattr(doc, "styles"):
        ...
        for style in doc.styles:
            ...
            pending.append((el,name,parent_el.get(qn("w:val"), "") if parent_el is not None else ""))
        for el,name,parent_id in pending:
            if parent_id:
                based_on[name]=id_to_name.get(parent_id,parent_id)
            _extract(el,name)
    else:
        root=_styles_root(doc)
        if root is not None:
            pending=[]
...
def _para_style_name(para) -> str:
    ...
    return style.name if style else "Normal"
