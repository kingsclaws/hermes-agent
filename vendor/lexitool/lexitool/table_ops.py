...
from .openxml_package import OpenXmlDocument
...
def extract_table(
    ...
    """
    doc = OpenXmlDocument(str(docx_path))
...
def fill_table(
    ...
    style_rPr_map = None
    try:
        from .format_brush import extract_style_rPr_map
    ...
    for row_idx, row_data in enumerate(data):
        ...
        for src_key, action, tgt_idx in mapped:
            ...
            fmt_check = _fill_cell_with_report(row.cells[tgt_idx], value, tc_id, author, date, as_tc_ins, preserve_rPr, style_rPr_map=style_rPr_map)
            if fmt_check and fmt_check.get("warnings"):
                format_warnings.append({"row": row_idx, "col": tgt_idx, "source_key": src_key, "warnings": fmt_check["warnings"], "context": fmt_check})
            tc_id += 1
        filled += 1

    return {"filled": filled, "format_warnings": format_warnings}
    ...
    for row in table.rows:
        ...
        for key_col, val_col in col_pairs:
            ...
            for frag_norm, val in expanded:
                ...
                if hit:
                    fmt_check = _fill_cell_with_report(row.cells[val_col], val, tc_id, author, date, as_tc_ins, preserve_rPr, style_rPr_map=style_rPr_map)
                    if fmt_check and fmt_check.get("warnings"):
                        format_warnings.append({"key_fragment": frag_norm, "row_cell": [key_col, val_col], "warnings": fmt_check["warnings"], "context": fmt_check})
                    tc_id += 1
                    filled += 1
...
def _fill_cell_with_report(cell, text: str, tc_id: int, author: str, date: str,
                           as_tc_ins: bool = True, preserve_rPr: bool = False,
                           style_rPr_map: dict | None = None) -> dict:
    ...
    if first_run is not None:
        rPr_el = first_run.find(qn("w:rPr"))
        if rPr_el is not None:
            before_rPr = deepcopy(rPr_el)
    _fill_cell(cell, text, tc_id, author, date, as_tc_ins=as_tc_ins, preserve_rPr=preserve_rPr)
    ins_nodes = para._element.findall(qn("w:ins")) if as_tc_ins else []
    target_run = None
    if ins_nodes:
        target = ins_nodes[-1]
        target_run = target.find(qn("w:r"))
    else:
        target_run = para._element.find(qn("w:r"))
    run_rPr = target_run.find(qn("w:rPr")) if target_run is not None else before_rPr
    return assess_para_format_context(para._element, run_rPr, style_rPr_map=style_rPr_map)
...
def copy_table(
    ...
    # ── 加载源文档 ──────────────────────────────────────────────────────────── #
    if isinstance(src_doc, (str, Path)):
        _src = OpenXmlDocument(str(src_doc))
    else:
        _src = src_doc
...
def inspect_table(
    ...
    if isinstance(doc_or_path, (str, Path)):
        doc = OpenXmlDocument(str(doc_or_path))
    else:
        doc = doc_or_path
    ...
    sample_rows = []
    for ri, row in enumerate(rows[:max(preview_rows, 0)]):
        row_cells = []
        for cell in row.cells:
            cell_text = _cell_text(cell)
            if preview_len and preview_len > 0 and len(cell_text) > preview_len:
                cell_text = cell_text[:preview_len] + "…"
            row_cells.append(cell_text)
        sample_rows.append({"row": ri, "cells": row_cells})
...
def table_format_brush(
    ...
    if isinstance(ref_doc_or_path, (str, Path)):
        ref_doc = OpenXmlDocument(str(ref_doc_or_path))
    else:
    ...
    for di, dst_row in enumerate(dst_rows):
        ...
        for ci, dst_cell in enumerate(dst_cells):
            ...
            for pi, dst_p in enumerate(dst_cell.paragraphs):
                ...
                dst_pPr = _ensure_pPr(dst_p._element)
                    ...
                    if src_runs:
                        ...
                        if src_rPr is not None:
                            ...
                            try:
                                from .format_brush import extract_style_rPr_map
                                from .tc_utils import assess_para_format_context
                                ctx = assess_para_format_context(dst_p._element, src_rPr, style_rPr_map=extract_style_rPr_map(dst_doc))
                                if ctx.get("warnings") or ctx.get("assumptions"):
                                    format_warnings.append({"row": di, "col": ci, "context": ctx})
                            except Exception:
                                pass
    ...
    return {"copied": applied, "format_warnings": format_warnings}


def replace_text_in_table_cell(
    doc,
    table_index: int,
    row_index: int,
    col_index: int,
    ...
    *,
    ...
    """在指定单元格段落内做细颗粒度 TC 替换。"""
    from .tc_utils import next_tc_id, tc_replace_first_in_para
    ...
    if paragraph_index >= len(cell.paragraphs):
        raise IndexError(
            f"cell paragraph_index out of range: table={table_index} row={row_index} col={col_index} para={paragraph_index}"
        )
    ...
    if replace_all:
        ...
        result = dict(combined[-1])
        result["replaced_count"] = len(combined)
    else:
        result = tc_replace_first_in_para(
            para._element,
            old_text=old_text,
    ...
    if result.get("ok"):
        ins_node = None
        for node in para._element.findall(qn("w:ins")):
            if node.get(qn("w:id")) == result.get("inserted_id"):
                ins_node = node
                break
        run_el = ins_node.find(qn("w:r")) if ins_node is not None else None
        ...
        result.update({
            ...
            "format_context": assess_para_format_context(para._element, run_rPr, style_rPr_map=style_rPr_map),
        })
    return result
