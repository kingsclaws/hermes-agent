...
class InjectPlan:
    ...
    jt_cell_notes:  TableCellNote 列表，在指定表格单元格末尾 append JT Note
    ...
    jt_cell_notes: list[TableCellNote] = field(default_factory=list)
...
def execute(plan: InjectPlan, cfg=None) -> InjectResult:
    ...
    result = InjectResult()
    doc = OpenXmlDocument(plan.doc_path)
    ...
    # ── 3. 表格单元格 JT Note 注入 ─────────────────────────────────────────── #
    for item in plan.jt_cell_notes:
        target = f"table={item.table_index},row={item.row_index},col={item.col_index},para={item.paragraph_index}"
        try:
            jt_note.append_to_table_cell(
                doc,
            ...
            result.notes.append({"key": target, "status": "ok"})
        except Exception as e:
            result.notes.append({"key": target, "status": f"error: {e}"})
