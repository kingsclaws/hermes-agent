...
class _InlineNoteCfg:
    def __init__(self, note_prefix: str, note_suffix: str, note_highlight: str):
        self.note_prefix = note_prefix
        self.note_suffix = note_suffix
        self.note_highlight = note_highlight
...
def append_to_paragraph(
    ...
    from .tc_utils import tc_ins_mixed
    tc_ins_mixed(
        para._element,
        [(prefix_space + note_text, True)],
        tc_id=tc_id,
        author=author,
        cfg=_InlineNoteCfg(note_prefix, note_suffix, highlight),
    )


def append_to_table_cell(
    doc,
    table_index: int,
    row_index: int,
    col_index: int,
    ...
    """
    在表格单元格指定段落末尾追加 JT Note，以 TC INS 形式注入。

    Args:
        table_index:      目标表格索引（0-based）
        row_index:        目标行索引（0-based）
        col_index:        目标列索引（0-based）
        note_text:        Note 正文（不含包裹标记，自动处理）
        paragraph_index:  单元格内目标段落索引，默认首段 0
    """
    author = author or _get_cfg_attr(cfg, "author", JT_AUTHOR)
    note_prefix = note_prefix or _get_cfg_attr(cfg, "note_prefix", JT_NOTE_PREFIX)
    note_suffix = note_suffix or _get_cfg_attr(cfg, "note_suffix", JT_NOTE_SUFFIX)
    ...
    if paragraph_index >= len(cell.paragraphs):
        raise IndexError(
            "cell paragraph_index out of range: "
            f"table={table_index} row={row_index} col={col_index} para={paragraph_index}"
        )
