...
def _style_name_from_id(style_info: dict, style_id: str | None) -> str:
    if not style_id:
        return "Normal"
    return style_info.get("by_id", {}).get(style_id, {}).get("name", style_id)
...
def _build_style_info(doc) -> dict:
    """构建样式信息；兼容 python-docx Document 与 OpenXML wrapper。"""

    def _extract_style_record(el, sid: str, name: str, parent_name: str | None):
    ...
    if hasattr(doc, "styles"):
        ...
        for style in doc.styles:
            ...
            parent_id = parent_el.get(qn("w:val"), "") if parent_el is not None else None
            styles_iter.append((el, sid, name, parent_id))
            by_id_name[sid] = name
        for el, sid, name, parent_id in styles_iter:
            parent_name = by_id_name.get(parent_id, parent_id) if parent_id else None
            raw[name] = _extract_style_record(el, sid, name, parent_name)
    else:
        styles_root = _styles_part_root(doc)
    ...
    result_by_id = {info["id"]: info for info in result_by_name.values() if info.get("id")}
    return {"by_name": result_by_name, "by_id": result_by_id}


def _para_style_name(para, style_info: dict | None = None) -> str:
    ...
    return _style_name_from_id(style_info or {"by_id": {}}, style_id)
...
def _effective_numpr(para, style_info: dict) -> dict | None:
    ...
    if own is not None:
        return own
    return style_info["by_name"].get(_para_style_name(para, style_info), {}).get("effective_numpr")
...
def _check_d04(doc, style_info: dict, para_range: tuple | None) -> list[Issue]:
    ...
    for pi in range(start, min(end, len(paras))):
        ...
        if _para_own_outline(para) is not None:
            continue   # 有显式设置，正常
        style_name = _para_style_name(para, style_info)
...
def _check_d05(doc, style_info: dict, para_range: tuple | None) -> list[Issue]:
    ...
    for pi in range(start, min(end, len(paras))):
        para = paras[pi]
        sid = by_name.get(_para_style_name(para, style_info), {}).get("id", "")
        if sid:
...
def _check_d08(doc, standards: Standards, style_info: dict,
    ...
    for pi in range(start, min(end, len(paras))):
        para = paras[pi]
        sname = _para_style_name(para, style_info)
...
def check(
    ...
    # 注意：Standards 新增字段时，这里必须同步透传，避免 check/fix 链路静默丢参。
    effective_standards = Standards(
        ...
        footer_blacklist=standards.footer_blacklist,
    )
