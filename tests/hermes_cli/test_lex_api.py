...
from hermes_cli.lex_api import router
...
def test_lex_api_projects_and_evidence(tmp_path, monkeypatch) -> None:
    ...
    listing = client.get("/api/lex/projects")
    ...
    evidence = client.get(f"/api/lex/projects/{pid}/evidence")
...
def test_lex_api_research_and_kanban(tmp_path, monkeypatch) -> None:
    ...
    research = client.get(f"/api/lex/projects/{pid}/research/runs")
    ...
    kanban = client.get(f"/api/lex/projects/{pid}/kanban")
    ...
    detail = client.get(f"/api/lex/projects/{pid}/research/runs/res_1")
    assert detail.status_code == 200
    detail_body = detail.json()
    assert detail_body["run"]["id"] == "res_1"

    cockpit = client.get(f"/api/lex/projects/{pid}/cockpit")
