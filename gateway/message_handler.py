"""
Message handler methods for GatewayRunner.

Extracted from gateway/run.py as a mixin class.
Contains _handle_message, _handle_message_with_agent, authorization,
and inbound message preparation.
"""
...
from gateway.config import Platform, GatewayConfig, HomeChannel, PlatformConfig
...
class MessageHandlerMixin:
    """Message handling methods extracted from GatewayRunner."""
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        if _update_prompts.get(_quick_key):
            ...
            if cmd in {"approve", "yes"}:
                ...
                if _recognized_cmd:
                    ...
                    response_text = raw
            if response_text:
                response_path = _gw._hermes_home / ".update_response"
        ...
        _raw_stale_timeout = _gw._float_env("HERMES_AGENT_TIMEOUT", 1800)
        ...
        if command and not locals().get("_bundle_handled", False):
            try:
                ...
                if cmd_key is not None:
                    ...
                    _unavail_msg = _gw._check_unavailable_skill(command)
                    if _unavail_msg:
                        return _unavail_msg
        ...
        *,
        ...
        if "@" in message_text:
            try:
                ...
                _msg_runtime = _gw._resolve_runtime_agent_kwargs()
                _msg_config_ctx = None
                try:
                    _msg_cfg = _gw._load_gateway_config()
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str, run_generation: int):
        ...
        _redact_pii = False
        try:
            _pcfg = _gw._load_gateway_config()
        ...
        if history and len(history) >= 4:
            ...
            _hyg_data = {}
            try:
                _hyg_data = _gw._load_gateway_config()
                if _hyg_data:
        ...
        try:
            ...
            if session_key and _gw._should_clear_resume_pending_after_turn(agent_result):
            ...
            response = _gw._normalize_empty_agent_response(
                agent_result, response, history_len=len(history),
            )
            response = _gw._sanitize_gateway_final_response(source.platform, response)
            ...
            try:
                ...
                _show_reasoning_effective = _rds(
                    _gw._load_gateway_config(),
                    _gw._platform_config_key(source.platform),
                    "show_reasoning",
