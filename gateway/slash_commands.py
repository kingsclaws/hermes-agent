...
Extracted from gateway/run.py as a mixin class to reduce file size.
All methods are instance methods on GatewayRunner via Python MRO.
"""
...
# Lazy access to gateway.run module-level names.
...
class SlashCommandsMixin:
    ...
    def _format_session_info(self) -> str:
        ...
        model = _gw._resolve_gateway_model()
        ...
        try:
            runtime = _gw._resolve_runtime_agent_kwargs()
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        try:
            ...
            atomic_json_write(
                _gw._hermes_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _gw._hermes_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    def _is_stale_restart_redelivery(self, event: MessageEvent) -> bool:
        ...
        try:
            marker_path = _gw._hermes_home / ".restart_last_processed.json"
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            pass
        return _gw._telegramize_command_mentions(
            "\n".join(lines),
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        user_provs = None
        custom_provs = None
        config_path = _gw._hermes_home / "config.yaml"
        try:
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected(
                        ...
                        _sw_config_ctx = None
                        try:
                            _sw_cfg = _gw._load_gateway_config()
        ...
        try:
            _sw2_cfg = _gw._load_gateway_config()
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        try:
            config = _gw._load_gateway_config()
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _gw._hermes_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        platform_key = _gw._platform_config_key(event.source.platform)
        if args in {"show", "on"}:
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> str:
        ...
        user_config = _gw._load_gateway_config()
        model = _gw._resolve_gateway_model(user_config)
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        # --- load config ----------------------------------------------------
        try:
            user_config: dict = _gw._load_gateway_config()
        except Exception as e:
        ...
        if new_state:
            ...
            preview = format_runtime_footer(
                model=_gw._resolve_gateway_model(user_config) or None,
                context_tokens=0,
                context_length=None,
